...
static uint8_t keyLength(uint8_t key)
{
    uint8_t count = 0;
...
static void totp_app_draw_callback(Canvas* canvas, void* ctx) {
    ...
    base32_decode(base32key[keyId], hmacKey, keyLength(base32key[keyId]));
    //FURI_LOG_I("TOTP", "len = %d", len);

    //uint8_t hmacKey[] = {0x48, 0x65, 0x6c, 0x6c, 0x6f, 0x21, 0xde, 0xad, 0xbe, 0xef}; // Secret key
    TOTP(hmacKey, keyLength(base32key[keyId]), 30); // Secret key, Secret key length, Timestep (30s)
